...
char* mm_typecode_to_str(MM_typecode matcode)
{
    ...
    /* check for MTX type */
    if (mm_is_matrix(matcode)) types[0] = (char*)MM_MTX_STR;
    // else
    //     error=1;
...
int mm_read_mtx_crd_data(FILE* f, int M, int N, int nz, int I[], int J[], double val[], MM_typecode matcode)
{
    ...
    if (mm_is_complex(matcode)) {
        for (i = 0; i < nz; i++)
            if (fscanf(f, "%d %d %lg %lg\n", &I[i], &J[i], &val[2 * i], &val[2 * i + 1]) != 4) return MM_PREMATURE_EOF;
    } else if (mm_is_real(matcode)) {

        for (i = 0; i < nz; i++) {

            if (fscanf(f, "%d %d %lg", &I[i], &J[i], &val[i]) != 3) return MM_PREMATURE_EOF;
        }
    }
